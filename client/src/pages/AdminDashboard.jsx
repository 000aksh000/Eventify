...
const AdminDashboard = () => {
    ...
    if (loading) {
        return <div className="text-center py-20 text-slate-500 font-medium">Loading admin dashboard...</div>;
    }
        ...
        <div className="max-w-7xl mx-auto space-y-8 py-8">
            ...
            <div className="neu-flat-lg rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-black text-[#25272a] tracking-tight">Admin Control Panel</h1>
                        <p className="text-xs font-semibold text-slate-500 mt-1">
                            Manage events, inspect revenue analytics, and approve ticket requests.
                        </p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="neu-flat p-6 rounded-3xl flex items-center justify-between">
                    <div className="space-y-1">
                        <p className="text-xs font-extrabold text-slate-500 uppercase tracking-wider">Total Revenue</p>
                        <h3 className="text-3xl font-black text-[#25272a]">₹{totalRevenue}</h3>
                    </div>
                ...
                <div className="neu-flat p-6 rounded-3xl flex items-center justify-between">
                    <div className="space-y-1">
                        <p className="text-xs font-extrabold text-slate-500 uppercase tracking-wider">Paid Clients</p>
                        <h3 className="text-3xl font-black text-[#25272a]">{paidClientsCount}</h3>
                    </div>
                    <div className="w-12 h-12 rounded-2xl neu-inset text-slate-700 flex items-center justify-center text-xl font-bold">
                ...
                <div className="neu-flat p-6 rounded-3xl flex items-center justify-between">
                    <div className="space-y-1">
                        <p className="text-xs font-extrabold text-slate-500 uppercase tracking-wider">Pending Requests</p>
                        <h3 className="text-3xl font-black text-[#25272a]">{pendingRequestsCount}</h3>
                    </div>
                    <div className="w-12 h-12 rounded-2xl neu-inset text-slate-700 flex items-center justify-center text-xl font-bold">
            ...
            {showEventForm && (
                <div className="neu-flat-lg p-8 rounded-3xl space-y-6">
                    <h2 className="text-xl font-black text-[#25272a]">Create & Publish Event</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                    ...
                    <div className="neu-flat rounded-3xl p-4 overflow-hidden">
                        <ul className="space-y-3 max-h-[550px] overflow-y-auto pr-1">
                            {events.length === 0 ? (
                                <li className="p-8 text-center text-xs text-slate-500 font-medium">No events created yet.</li>
                            ) : (
                                events.map((event) => (
                                    <li
                                        key={event._id}
                                    ...
                                    >
                                        <div className="space-y-1 min-w-0">
                                            <h4 className="font-bold text-[#25272a] text-xs truncate">{event.title}</h4>
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                            {confirmDeleteId === event._id ? (
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    >
                                                        {deletingId === event._id ? 'Deleting...' : 'Confirm Delete'}
                                                    </button>
                                                    <button
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between border-b border-slate-300/40 pb-3">
                        <h2 className="text-base font-bold text-[#25272a] flex items-center gap-2">
                            <HiClock className="w-5 h-5 text-slate-600" /> Ticket Booking Requests
                        </h2>
                        <span className="text-xs font-bold neu-badge px-3 py-1 rounded-full text-slate-700">
                            {bookings.length}
                        </span>
                    ...
                    <div className="neu-flat rounded-3xl p-4 overflow-hidden">
                        <ul className="space-y-4 max-h-[550px] overflow-y-auto pr-1">
                            {bookings.length === 0 ? (
                                <li className="p-8 text-center text-xs text-slate-500 font-medium">No booking requests yet.</li>
                            ) : (
                                bookings.map((booking) => (
                                    <li key={booking._id} className="p-5 rounded-2xl neu-inset space-y-3">
                                        ...
                                        <div className="space-y-1 text-xs text-slate-600 font-semibold">
                                            <p className="flex justify-between">
                                                ...
                                                <span className="text-slate-800">{booking.userId?.name} ({booking.userId?.email})</span>
                                            </p>
                                            <p className="flex justify-between">
                                                <span className="text-slate-400">Amount:</span>
                                                <span className="text-slate-800">{booking.amount === 0 ? 'Free' : `₹${booking.amount}`}</span>
                                            </p>
                                            <p className="flex justify-between">
