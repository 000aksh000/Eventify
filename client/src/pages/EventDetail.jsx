...
const EventDetail = () => {
    ...
    const handleBooking = async () => {
        ...
        try {
            if (!showOTP) {
                ...
                setShowOTP(true);
                setSuccessMsg('OTP sent to your email address. Enter the code below to confirm booking.');
            } else {
                await api.post('/bookings', { eventId: event._id, otp });
                setSuccessMsg('Booking requested successfully! Your ticket request is pending admin approval.');
                setShowOTP(false);
    ...
    if (loading) {
        return <div className="text-center py-20 text-slate-500 font-medium">Loading event details...</div>;
    }
    if (error && !event) {
        return <div className="text-center py-20 text-rose-500 font-medium">{error || 'Event not found'}</div>;
    }
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-start gap-8">
                        ...
                        <div className="w-full lg:w-80 neu-flat p-6 rounded-2xl shrink-0 space-y-6">
                            ...
                            <div className="space-y-4 text-xs font-semibold">
                                ...
                                <div className="flex items-center justify-between">
                                    <span className="text-slate-500 flex items-center gap-1.5">
                                        <HiUserGroup className="w-4 h-4 text-slate-400" /> Availability
                                    </span>
                                    <span className="font-bold text-slate-700">
                                        <span className={event.availableSeats < 10 ? 'text-amber-700 font-black' : ''}>
