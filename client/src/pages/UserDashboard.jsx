...
import { HiTicket, HiXMark, HiClock, HiArrowRight, HiCalendar, HiCurrencyRupee } from 'react-icons/hi2';
...
const UserDashboard = () => {
    ...
    const cancelBooking = async (id) => {
        if (window.confirm('Are you sure you want to cancel this ticket booking request?')) {
            try {
    ...
    if (loading) {
        return <div className="text-center py-20 text-slate-500 font-medium">Loading your dashboard...</div>;
    }
        ...
        <div className="max-w-6xl mx-auto space-y-8 py-8">
                ...
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-xs font-semibold text-slate-500 flex items-center gap-2 mt-1">
                            <span className="w-2 h-2 rounded-full bg-slate-600"></span> Verified Account Dashboard
                        </p>
                    </div>
                </div>
            ...
            {bookings.length === 0 ? (
                <div className="neu-flat rounded-3xl p-12 text-center space-y-4">
                    ...
                    <div>
                        <h3 className="text-base font-bold text-[#25272a]">No ticket requests found</h3>
                        <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1 font-medium">
                            You haven't reserved any event tickets yet. Explore upcoming events and reserve your seats now.
                        </p>
                    </div>
                    <Link
                        to="/"
                        ...
                        >
                            <div className="p-6 space-y-4">
                                {booking.eventId ? (
                                    <>
                                        ...
                                        <div className="space-y-2 text-xs font-semibold text-slate-600 neu-inset p-3.5 rounded-xl">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span>{booking.amount === 0 ? 'Free Ticket' : `₹${booking.amount}`}</span>
                                            </div>
                                    ...
                                    <p className="text-xs text-rose-600 italic">Event details unavailable</p>
                                )}
                            </div>
                            ...
                            <div className="p-4 border-t border-slate-300/30 flex items-center justify-between text-xs font-bold">
                                {booking.eventId && booking.status !== 'cancelled' ? (
                                    ...
                                    <span className="text-slate-400 italic mx-auto">Booking Cancelled</span>
                                )}
                            </div>
                        </div>
                    ))}
